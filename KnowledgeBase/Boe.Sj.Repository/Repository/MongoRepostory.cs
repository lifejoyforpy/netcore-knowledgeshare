...
namespace BoeSj.KnowledgeBase.Infrastructure.Mongo
{
    ...
    CancellationToken cancellationToken = new CancellationToken()) where TEntity : class, IMongoEntity<string>
        ...
        public async Task<long> UpdateAsync<TEntity>(Expression<Func<TEntity, bool>> predicate, object entity,
    CancellationToken cancellationToken = new CancellationToken()) where TEntity : class, IMongoEntity<string>
        {
            var doc = entity.ToBsonDocument();
            var newDocument =
            new BsonDocument { { "$set", doc } };
            var result = await this.database.GetCollection<TEntity>(typeof(TEntity).Name).UpdateManyAsync(predicate, newDocument);
            return result.ModifiedCount;
        }
        /// <summary>
        /// </summary>
